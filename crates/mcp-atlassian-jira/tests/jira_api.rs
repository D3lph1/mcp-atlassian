...
use mcp_atlassian_jira::{CreateIssueParams, JiraClient, SearchParams};
use serde_json::json;
...
async fn get_issue_requests_the_standard_set_and_models_structure() {
    ...
    Mock::given(method("GET"))
        .and(path("/rest/api/2/issue/PROJ-1"))
        .and(query_param("fields", mcp_atlassian_jira::DEFAULT_ISSUE_FIELDS))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            "id": "10001", "key": "PROJ-1",
            "fields": {
                "summary": "Child",
...
async fn an_explicit_deployment_overrides_the_auth_mode_inference() {
    ...
    let jira = JiraClient::new(&ServiceConfig {
        ...
        deployment: Some(mcp_atlassian_client::Deployment::Server),
    })
    .unwrap();
